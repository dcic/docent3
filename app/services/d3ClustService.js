...
(function() {
  ...
  function d3Clust(d3, lodash) {
    ...
    function initialize_visualization(network_data, params) {
      ...
      var label_scale = d3.scale.linear().domain([min_num_char,
        max_num_char
      ...
      params.class_room = {};
      if (lodash.has(params, 'group_colors')) {
      ...
      params.norm_label.margin.left = params.grey_border_width + params
        .super_label_width;
      ...
      var prevent_col_stretch = d3.scale.linear().domain([1, 20]).range(
        [0.05, 1]).clamp('true');
      ...
      params.x_scale = d3.scale.ordinal().rangeBands([0, params.clust.dim
        .width
      ]);
      params.y_scale = d3.scale.ordinal().rangeBands([0, params.clust.dim
        .height
      ]);
      ...
      var scale_font_size = d3.scale.log().domain([min_node_num,
        max_node_num
      ]).range([max_fs, min_fs]).clamp('true');
      ...
      var scale_reduce_font_size_factor = d3.scale.log().domain([
      ...
      var scale_fs_screen_width = d3.scale.linear().domain([
        min_viz_width, max_viz_width
      ]).range([0.75, 1.15]).clamp('true');
      var scale_fs_screen_height = d3.scale.linear().domain([
        min_viz_width, max_viz_width
      ]).range([0.75, 1.15]).clamp('true');

      // the default font sizes are set here
      params.default_fs_row = scale_font_size(row_nodes.length) *
        scale_fs_screen_height(params.clust.dim.height);
      // the colum font size is scaled by the width
      //!! make this local later
      params.default_fs_col = scale_font_size(col_nodes.length) *
        scale_fs_screen_width(params.clust.dim.width);
      ...
      params.reduce_font_size.row = scale_reduce_font_size_factor(
        row_nodes.length);
      params.reduce_font_size.col = scale_reduce_font_size_factor(
        col_nodes.length);
      ...
      var real_zoom_scale_screen = d3.scale.linear().domain([
        min_viz_width, max_viz_width
      ...
      // if ( lodash.has(network_data.links[0], 'value_up') || lodash.has(network_data.links[0], 'highlight') ){
      if (lodash.has(network_data.links[0], 'value_up')) {
      ...
      if (lodash.has(d3_clustergram.network_data.links[0], 'highlight')) {
        params.highlight = 1;
    ...
    function make_d3_clustergram(args) {
      ...
      params.rand_colors = ['#000000', '#FF34FF', '#FFFF00', '#FF4A46',
        ...
        '#00CCFF', '#674E60', '#FC009C', '#92896B', '#1CE6FF'
      ];
      ...
      if (lodash.has(row_nodes[0], 'group') === true || lodash.has(col_nodes[0],
          'group')) {
      ...
      if (lodash.has(row_nodes[0], 'cl') || lodash.has(col_nodes[0], 'cl')) {
        // gather classes
        params.class_colors = {};
      }

      // gather class information from row
      if (lodash.has(row_nodes[0], 'cl') === true) {
        var class_row = lodash.uniq(lodash.pluck(row_nodes, 'cl'));
        // associate classes with colors
        params.class_colors.row = {};
        for (var i = 0; i < class_row.length; i++) {
          params.class_colors.row[class_row[i]] = params.rand_colors[i +
            50 % num_colors];
        }
      }
      // gather class information from col
      if (lodash.has(col_nodes[0], 'cl') === true) {
        var class_col = lodash.uniq(lodash.pluck(col_nodes, 'cl'));
        // associate classes with colors
        ...
        for (var i = 0; i < class_col.length; i++) {
          if (i === 0) {
            ...
            params.class_colors.col[class_col[i]] = params.rand_colors[
              i + 50 % num_colors];
          }
        }
      }
      ...
      if (lodash.has(row_nodes[0], 'group') === true) {
        ...
        for (var i = 0; i < 200; i++) {
          ...
          if (i === 1) {
            ...
            params.group_colors.row[i] = params.rand_colors[i %
              num_colors];
          }
        }
      }
      ...
      if (lodash.has(col_nodes[0], 'group') === true) {
        ...
        for (var i = 0; i < 200; i++) {
          ...
          if (i === 1) {
            ...
            params.group_colors.col[i] = params.rand_colors[i %
              num_colors];
          }
        }
      }
      ...
      if (params.tile_type === 'simple') {
        ...
        network_data.links.forEach(function(link) {
          ...
          if (params.highlight === 1) {
            params.matrix[link.source][link.target].highlight =
              link.highlight;
          }
      ...
      if (params.tile_type === 'simple') {
        ...
        row_obj = params.clust_group.selectAll('.row')
          .data(params.matrix)
          .enter()
          .append('g')
          .attr('class', 'row')
      ...
      row_triangle_ini_group
        ...
        .attr('d', function() {
          ...
          var output_string = 'M ' + origin_x + ',' + origin_y +
            ' L ' + mid_x + ',' + mid_y + ', L ' + final_x + ',' +
            final_y + ' Z';
        ...
        .attr('fill', function(d) {
          // initailize color
          var inst_color = '#eee';
          if (lodash.has(params, 'class_colors')) {
        ...
        if (typeof params.click_group === 'function') {
          ...
          row_class_rect
            .on('click', function(d) {
              ...
              var group_nodes = [];
              for (var index = 0; index < row_nodes.length; index++) {
                // check that the node is in the group
                if (row_nodes[index].group[inst_level] === inst_group) {
                  // make a list of genes that are in inst_group at this group_level
                  group_nodes.push(row_nodes[index].name);
                }
              }
      ...
      var col_label_click = col_label_obj
        ...
        .attr('transform', 'translate(' + params.x_scale.rangeBand() /
      ...
      col_label_click
        ...
        .attr('d', function(d) {
          ...
          var final_x = params.x_scale.rangeBand() -
            reduce_rect_width;
          var start_y = -(params.x_scale.rangeBand() -
            reduce_rect_width + params.border_width);
      ...
      // enr_max = lodash.max( col_nodes, function(d) { return Math.abs(d.nl_pval) } ).nl_pval ;
      ...
      if (lodash.has(d3_clustergram.network_data.col_nodes[0], 'value')) {
        col_label_click
          .append('rect')
          .attr('class', 'col_bars')
      ...
      if (params.super_labels === 'yes') {
        ...
        d3.select('#main_svg')
          ...
          .attr('height', '3000px')
          .attr('class', 'white_bars')
          .attr('transform', 'translate(' + params.grey_border_width +
            ',0)');
      ...
      params.zoom.translate([params.clust.margin.left, params.clust.margin
        .top
      ]);
    ...
    function highlight_resource_types(params) {
      ...
      // var all_groups = lodash.keys(params.class_colors.row);
    ...
    function row_function(row_data) {
      ...
      row_data = lodash.filter(row_data, function(num) {
        return num.value !== 0;
      });
      ...
      var tile = d3.select(this)
        ...
        .style('fill', function(d) {
          return d.value > 0 ? params.tile_colors[0] : params.tile_colors[
            1];
        })
      ...
      if (typeof params.click_tile === 'function') {
        d3.selectAll('.tile')
          .on('click', function(d) {
            ...
            var tile_info = {};
            tile_info.row = d3_clustergram.network_data.row_nodes[d.pos_y]
              .name;
            tile_info.col = d3_clustergram.network_data.col_nodes[d.pos_x]
              .name;
    ...
    function row_group_function(row_data) {
      ...
      tile
        ...
        .style('fill', function(d) {
          ...
          return d.value_dn < 0 ? params.tile_colors[0] : params.tile_colors[
            1];
        });
      ...
      if (params.highlight === 1) {
        ...
        tile
          ...
          .attr('transform', 'translate(' + params.x_scale.rangeBand() /
            10 + ' , ' + params.y_scale.rangeBand() / 10 + ')')
      ...
      tile
        ...
        .attr('d', function() {
          ...
          var final_y = params.y_scale.rangeBand() - params.y_scale.rangeBand() /
            60;
          var output_string = 'M' + start_x + ', ' + start_y +
            ' ,       L' + final_x + ', ' + final_y + ',  L' +
            final_x + ',0 Z';
          return output_string;
        })
      ...
      if (params.title_tile === true) {
        tile
          ...
          .text(function(d) {
            // var inst_string = 'value: ' + d.value ;
            // specific to LDR
            var inst_string = 'perturbations: ' + d.value +
              '; released: ' + d.value_up + '; not-released: ' + Math
              .abs(d.value_dn);
            return inst_string;
          });
      }
    ...
    function reset_visualization_size() {
      ...
      d3_clustergram.params.zoom.scale(1).translate([d3_clustergram.params
        .clust.margin.left, d3_clustergram.params.clust.margin.top
      ]);
    ...
    function zoomed() {
      ...
      var trans_x = d3.event.translate[0] - d3_clustergram.params.clust
        .margin.left;
      var trans_y = d3.event.translate[1] - d3_clustergram.params.clust
        .margin.top;
    ...
    function apply_transformation(trans_x, trans_y, zoom_x, zoom_y) {
      ...
      // scale is greater than params.zoom_switch
        ...
        // multiple extra room (zoom - 1) by the width
        var pan_room_x = (d3_scale / params.zoom_switch - 1) * params.clust
          .dim.width;
      ...
      params.zoom
        .translate([trans_x + params.clust.margin.left, trans_y +
          params.clust.margin.top
        ]);
      ...
      var reduce_font_size = d3.scale.linear().domain([0, 1]).range([1,
        zoom_y
      ]).clamp('true');
      // scale down the font to compensate for zooming
      var fin_font = params.default_fs_row / (reduce_font_size(params.reduce_font_size
        .row));
      ...
      reduce_font_size = d3.scale.linear().domain([0, 1]).range([1,
        zoom_x
      ]).clamp('true');
      // scale down the font to compensate for zooming
      fin_font = params.default_fs_col / (reduce_font_size(params.reduce_font_size
        .col));
      // add back the 'px' to the font size
      fin_font = fin_font + 'px';
      // change the font size of the labels
    ...
    function two_translate_zoom(pan_dx, pan_dy, fin_zoom) {
      ...
      if (d3_clustergram.params.run_trans === 0) {
        ...
        if (pan_dy < -(half_height - y_pan_room)) {
          ...
          shift_up_viz = half_height / params.zoom_switch -
        ...
        d3.select('#row_labels')
          ...
          .attr('transform', 'translate(' + [0, center_y] + ')' +
            ' scale(' + zoom_y + ',' + zoom_y + ')' + 'translate(' + [0,
              pan_dy
            ] + ')');
        ...
        // reduce font-size to compensate for zoom
        // calculate the recuction of the font size
        var reduce_font_size = d3.scale.linear().domain([0, 1]).range([
          1, zoom_y
        ]).clamp('true');
        // scale down the font to compensate for zooming
        var fin_font = params.default_fs_row / (reduce_font_size(params
          .reduce_font_size.row));
        // add back the 'px' to the font size
        fin_font = fin_font + 'px';
        // change the font size of the labels
        d3.selectAll('.row_label_text')
        ...
        // reduce font-size to compensate for zoom
        // calculate the recuction of the font size
        reduce_font_size = d3.scale.linear().domain([0, 1]).range([
          1, zoom_x
        ]).clamp('true');
        // scale down the font to compensate for zooming
        fin_font = params.default_fs_col / (reduce_font_size(params
          .reduce_font_size.col));
        // add back the 'px' to the font size
        fin_font = fin_font + 'px';
        // change the font size of the labels
        d3.selectAll('.col_label_text')
    ...
    function reorder_click_row(d, i) {
      ...
      // find index
      var inst_row = lodash.indexOf(tmp_arr, inst_gene);
    ...
    function zoom_and_highlight_found_gene(search_gene) {
      ...
      var inst_gene_index = lodash.indexOf(params.all_genes, search_gene);
